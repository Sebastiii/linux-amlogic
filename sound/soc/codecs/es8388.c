...
struct es8388_priv {
	...
	struct workqueue_struct *Aux_det_wq;
...
#if 0
static irqreturn_t aux_det_irq_handler(int irq, void *dev_id)
{
	int ret;
	unsigned int type;
	struct es8388_priv *es8388 = es8388_private;
	...
	type = gpio_get_value(es8388->aux_det_gpio) ? IRQ_TYPE_EDGE_FALLING : IRQ_TYPE_EDGE_RISING;
	ret = irq_set_irq_type(irq, type);
	if (ret < 0) {
		pr_err("%s: irq_set_irq_type(%d, %d) failed\n", __func__, irq, type);
		return -1;
	...
	if(es8388->aux_det_level == gpio_get_value(es8388->aux_det_gpio)){
		DBG("%s--aux_det_level = 0,insert aux\n",__FUNCTION__);
		es8388_set_mic_channel(0);
	}else{
		DBG("%s--aux_det_level = 1,deinsert aux\n",__FUNCTION__);
		es8388_set_mic_channel(1);
	}
...
#else
static unsigned int es8388_read_reg(struct snd_soc_codec *codec,
				     unsigned int reg)
{
	unsigned int ret;
	u8 read_cmd[3] = {0};
...
//		pr_err("es8388_read client->adapter==NULL\n");
	...
	ret = i2c_master_send(codec->control_data, read_cmd, cmd_len);
	if (ret != cmd_len) {
		pr_err("es8388_read error1\n");
		return -1;
	}

	ret = i2c_master_recv(codec->control_data, rt_value, 1);
	if (ret != 1) {
...
//	BUG_ON(codec->volatile_register);
	...
	ret = i2c_master_send(codec->control_data, write_cmd, 2);
	if (ret != 2) {
...
static void es8388_set_mic_channel(int channel)
{
	DBG("%s: channel=%d\n",__FUNCTION__, channel);
	if(channel==0)
	{
		es8388_write(es8388_codec, ES8388_ADCCONTROL1, 0x00);
		es8388_write(es8388_codec, ES8388_ADCCONTROL2, 0x00);
	}
	else
	{
		es8388_write(es8388_codec, ES8388_ADCCONTROL1, 0x80);
		es8388_write(es8388_codec, ES8388_ADCCONTROL2, 0xf0);
	}
}
...
static void delay_work_func(struct work_struct *work)
{
	...
	es8388 = container_of(dwork, struct es8388_priv, Aux_det_work);

	ret = gpio_get_value(es8388->aux_det_gpio);

	aux_level = es8388->aux_det_level;
	if(aux_level!=ret)
	{
		es8388_set_mic_channel(ret);
		es8388->aux_det_level = ret;
	}

	queue_delayed_work(es8388->Aux_det_wq, &es8388->Aux_det_work, 300);
}
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		...
		snd_soc_write(codec, ES8388_ANAVOLMANAG, 0x7a);
...
*/
	codec->hw_write = (hw_write_t)i2c_master_recv;//i2c_master_send;
//	codec->control_data = container_of(codec->dev, struct i2c_client, dev);
	codec->control_data = es8388->control_data;

	es8388_codec = codec;
	ret = es8388_reset(codec);
	if (ret < 0) {
		dev_err(codec->dev, "%s--Failed to issue reset\n",__FUNCTION__);
//		return ret;
	}
	else
	...
	msleep(100);
	ret = snd_soc_write(codec, 0x02,0xf3);
 	if(ret)
 		DBG("%s-- snd_soc_write failed!\n",__FUNCTION__);
 	else
 		DBG("%s-- snd_soc_write success!\n",__FUNCTION__);
	...
	snd_soc_write(codec, 0x03,0x00);   //ori:0x59 yangjr//pdn_ana=0,ibiasgen_pdn=0
	snd_soc_write(codec, 0x05,0xe8);   //pdn_ana=0,ibiasgen_pdn=0
	snd_soc_write(codec, 0x06,0xc3);   //pdn_ana=0,ibiasgen_pdn=0
	snd_soc_write(codec, 0x07,0x7a);
	snd_soc_write(codec, 0x09,0x80);  //ADC L/R PGA =  +24dB
	snd_soc_write(codec, 0x0a,0xf0);  //ori:0xf0 yangjr//ADC INPUT=LIN2/RIN2
	snd_soc_write(codec, 0x0b,0x80);  //ADC INPUT=LIN2/RIN2 //82
	snd_soc_write(codec, 0x0C,0x0C);  //ori:0x4c//I2S-24BIT
	...
	snd_soc_write(codec, 0x12,0x00); // ALC stereo MAXGAIN: 35.5dB,  MINGAIN: +6dB (Record Volume increased!)
	snd_soc_write(codec, 0x13,0xc0);
	snd_soc_write(codec, 0x14,0x05);
	snd_soc_write(codec, 0x15,0x06);
	snd_soc_write(codec, 0x16,0x93);
	snd_soc_write(codec, 0x17,0x18);  //ori:0x18 yangjr//I2S-16BIT
	snd_soc_write(codec, 0x18,0x02);
	snd_soc_write(codec, 0x1A,0x00);  //ori:0x0A yangjr//DAC VOLUME=0DB
	snd_soc_write(codec, 0x1B,0x00);  //ori:0x0A yangjr
    /*
    ...
    */
	snd_soc_write(codec, 0x26,0x12);  //ori:0x12,yangjr//Left DAC TO Left IXER
	snd_soc_write(codec, 0x27,0xb8);  //ori:0xb8,yangjr//Left DAC TO Left MIXER
	snd_soc_write(codec, 0x28,0x38);
	snd_soc_write(codec, 0x29,0x38);
	snd_soc_write(codec, 0x2A,0xb8);	//ori:0xb8,yangjr
	...
	msleep(200);
	snd_soc_write(codec, 0x2e,0x1e); //ori:0x18
	snd_soc_write(codec, 0x2f,0x1e); //ori:0x18
	snd_soc_write(codec, 0x30,0x1e); //ori:0x18
	snd_soc_write(codec, 0x31,0x1e); //ori:0x18

	//snd_soc_write(codec, ES8388_DACCONTROL3, 0x06); //yangjr  [2]:1 mute analog outputs for both channels
	#endif
...
*/
	...
	es8388->aux_det_gpio = of_get_named_gpio_flags(i2c->dev.of_node, "aux-det-gpio", 0, &flags);
	if (es8388->aux_det_gpio < 0) {
		DBG("%s() Can not read property aux_det gpio\n", __FUNCTION__);
		es8388->aux_det_gpio = INVALID_GPIO;
	}
	else
	{
		DBG("%s() aux_det_gpio:%d OK!\n", __FUNCTION__,es8388->aux_det_gpio);
//		es8388->aux_det_level = (flags & OF_GPIO_ACTIVE_LOW)? 0:1;
		ret = gpio_request(es8388->aux_det_gpio, NULL);
		if (ret != 0) {
			printk("%s request AUX_DET error", __func__);
			return ret;
		}
		else
			DBG("%s() gpio_request aux_det_gpio OK!\n", __FUNCTION__);
		ret = gpio_direction_input(es8388->aux_det_gpio);
	//	ret = gpio_direction_output(es8388->aux_det_gpio,1);
		DBG("%s() gpio_direction_input aux_det_gpio:%d!\n", __FUNCTION__,ret);

		if (ret) {
			printk("%s gpio_direction_input AUX_DET error", __func__);
			return ret;
		}
		else
			DBG("%s() gpio_direction_input aux_det_gpio OK!\n", __FUNCTION__);
//		ret = gpio_get_value(es8388->aux_det_gpio);
		es8388->aux_det_level = 1;
//		es8388_set_mic_channel(ret);

		es8388->Aux_det_wq = create_workqueue("my workqueue");
		if (!es8388->Aux_det_wq) {
			printk(KERN_ERR "Create workqueue failed!\n");
			return -1;
		}

		INIT_DELAYED_WORK(&es8388->Aux_det_work, delay_work_func);
		ret = queue_delayed_work(es8388->Aux_det_wq, &es8388->Aux_det_work, 3000);

#if 0
		//irq_flag = IRQF_TRIGGER_LOW |IRQF_ONESHOT;
		irq_flag = IRQF_TRIGGER_RISING |IRQF_TRIGGER_FALLING;
		aux_irq = gpio_to_irq(es8388->aux_det_gpio);

	    if (aux_irq){
		ret = request_threaded_irq(aux_irq, NULL, aux_det_irq_handler, irq_flag, "ES8388", NULL);
		if(ret == 0){
		    printk("%s:register ISR (irq=%d)\n", __FUNCTION__,aux_irq);
		}
		else
			printk("%s:request_irq aux_irq failed\n",__FUNCTION__);
	    }
#endif
	}
